...
<body>
    ...
    <div id="main-section" class="hidden flex h-screen">
        <aside class="w-72 sidebar text-white flex-shrink-0 flex flex-col">
            ...
            <nav class="flex-1 px-4 py-6 space-y-1">
                <div onclick="showPage('dashboard')" id="btn-dashboard" class="sidebar-link sidebar-active flex items-center gap-3 p-4 text-sm font-semibold">
                    <i class="fas fa-chart-line w-5"></i> Dashboard
                </div>
                <div onclick="showPage('stok')" id="btn-stok" class="sidebar-link flex items-center gap-3 p-4 text-sm font-semibold">
                    <i class="fas fa-boxes w-5"></i> Parça Yönetimi
                </div>
                <div onclick="showPage('fatura')" id="btn-fatura" class="sidebar-link flex items-center gap-3 p-4 text-sm font-semibold">
                    <i class="fas fa-file-invoice w-5"></i> Fatura Kes
                </div>
                <div onclick="showPage('cari')" id="btn-cari" class="sidebar-link flex items-center gap-3 p-4 text-sm font-semibold">
                    <i class="fas fa-users w-5"></i> Cari Kayıtlar
                </div>
                <div onclick="showPage('gider')" id="btn-gider" class="sidebar-link flex items-center gap-3 p-4 text-sm font-semibold">
                    <i class="fas fa-wallet w-5"></i> Harcamalar
                </div>
            ...
            <div class="p-6 border-t border-slate-800">
                <button onclick="handleLogout()" class="w-full p-3 text-slate-400 hover:text-red-400 text-xs font-bold uppercase transition-all">
        ...
        <main class="flex-1 relative overflow-hidden">
            <div id="page-dashboard" class="page-content active-section">
                <div class="flex justify-between items-end mb-10 pb-6 border-b border-slate-200">
                    <div>
                        <h3 class="text-3xl font-extrabold text-slate-800 uppercase italic">Sistem Paneli</h3>
                        <p id="current-date" class="text-slate-500 font-medium mt-1 text-sm"></p>
                    </div>
                    <div class="text-right">
                        <h4 class="text-lg font-bold text-slate-700 italic">Hoş geldin Eren Usta</h4>
                        <p class="text-slate-400 font-semibold text-[10px] uppercase tracking-widest">Özcanoto yedek parça stok</p>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    <table class="w-full text-left text-sm">
                        <thead class="bg-slate-50 border-b border-slate-200 text-slate-500 font-bold uppercase text-[10px]">
                            <tr>
                                <th class="px-8 py-5">Parça Tanımı</th>
                                <th class="px-8 py-5 text-center">Stok Durumu</th>
                                <th class="px-8 py-5 text-right">Birim Fiyat</th>
    ...
    <script>
            ...
            document.getElementById('stok-list').innerHTML = products
                ...
                .map(i => {
                    ...
                    return `<tr class="${isCrit ? 'critical-row' : ''}">
                        ...
                        <td class="px-8 py-5 text-right"><button onclick="sil('${i._id}','products')" class="text-slate-300 hover:text-red-600"><i class="fas fa-trash-alt"></i></button></td>
                    </tr>`;
                }).join('');
        }
        ...
        async function giderEkle() {
            ...
            if(!ad || !t) return alert("Eksik veri!");
