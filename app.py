...
# ✅ STOK + / - (DÜZELTİLDİ)
# ===============================
@app.route('/api/products/update-stock/<id>', methods=['POST'])
def update_stock(id):
    data = request.json
    miktar = int(data.get('change', 0))
...
def toplu_satis():
    return jsonify(
        satis_yonetimi.toplu_fatura_kes(db, request.json)
    )
...
def get_stats():
    ...
    depo = sum(
        int(p.get('stock', 0)) * temizle(p.get('price', 0))
        for p in products
    )
